...
const TICKER_SYMBOLS = ['BTC', 'ETH', 'BNB', 'SOL', 'XRP', 'DOGE', 'ADA', 'DOT', 'MATIC', 'LINK'];
...
const SAMPLE_NEWS = [
  {
    ...
    summary: 'Major financial institutions continue to increase their Bitcoin holdings, driving prices to new highs this quarter.',
    time: '2h ago',
    tag: 'Bitcoin',
  },
  {
    title: 'Ethereum Layer 2 Adoption Reaches Record Levels',
    summary: 'Transaction volumes on Ethereum L2 networks have surpassed 50 million daily transactions for the first time.',
    time: '5h ago',
    tag: 'Ethereum',
  },
  {
    title: 'Central Banks Accelerate Digital Currency Research',
    summary: 'Over 130 countries are now exploring central bank digital currencies, with 19 in advanced pilot stages.',
    time: '8h ago',
    tag: 'Regulation',
  },
];
  ...
  if (active && payload && payload.length) {
    return (
      <div className="bg-white rounded-lg shadow-lg border border-sky-100 px-3 py-2">
        ...
        <p className="text-sm font-bold text-gray-800">{formatCurrency(payload[0].value)}</p>
      </div>
    );
  }
  return null;
...
export default function Dashboard() {
  ...
  const totalTrades = tradeHistory.length > 0 ? tradeHistory.length : (user?.total_trades ?? 0);
  const totalPL = user?.total_pl ?? user?.total_profit ?? 0;
  ...
  useEffect(() => {
    ...
    const update = () => {
      ...
      activeTrades.forEach((trade) => {
        const rawEnd = trade.expires_at || trade.end_time || (trade.created_at && new Date(trade.created_at).getTime() + (trade.duration || 0) * 1000);
        const end = new Date(rawEnd).getTime();
        next[trade.id || trade._id] = Math.max(0, Math.ceil((end - Date.now()) / 1000));
      });
      setCountdowns(next);
    };
    update();
    ...
    try {
      ...
      Object.entries(COINGECKO_MAP).forEach(([symbol, id]) => {
        ...
        normalized[symbol] = {
          ...
          change_24h: raw.usd_24h_change ?? raw.change_24h ?? raw.change ?? 0,
          volume: raw.usd_24h_vol ?? raw.volume ?? raw.quote_volume ?? 0,
        };
      });
  ...
  function getCreditBadge() {
    if (creditScore >= 700) {
      return { label: 'Premium', icon: Crown, iconColor: 'text-sky-500', labelColor: 'text-sky-600', bg: 'bg-sky-50 border-sky-200' };
    }
    if (creditScore >= 400) {
      return { label: 'Trusted', icon: Shield, iconColor: 'text-emerald-500', labelColor: 'text-emerald-600', bg: 'bg-emerald-50 border-emerald-200' };
    }
    return { label: 'Standard', icon: Star, iconColor: 'text-gray-400', labelColor: 'text-gray-500', bg: 'bg-gray-50 border-gray-200' };
  }
  ...
  return (
    <div className="min-h-screen relative overflow-hidden">
      <div className="fixed inset-0 pointer-events-none z-0">
        ...
        >
          <span className="font-heading font-bold text-sky-500 whitespace-nowrap" style={{ opacity: 0.03 }}>
            Gemini Exchange
          </span>
        </div>
      ...
      <div className="relative z-10 p-4 md:p-6 lg:p-8 max-w-7xl mx-auto space-y-6">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-3 flex-wrap">
                ...
                <span className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-medium border ${creditBadge.bg}`}>
                  <CreditIcon className={`w-3 h-3 ${creditBadge.iconColor}`} />
                  <span className={creditBadge.labelColor}>{creditBadge.label}</span>
                </span>
              </div>
        ...
        <div className="animate-fade-in bg-white rounded-xl shadow-sm border border-sky-100 p-6 flex flex-wrap items-center justify-between gap-4" style={{ animationDelay: '0.1s' }}>
          <div>
            <p className="text-gray-400 text-xs font-medium uppercase tracking-widest mb-1">Portfolio Value</p>
            <p className="font-heading text-3xl font-bold text-gray-800">
              {formatCurrency(balance)} <span className="text-lg text-gray-400 font-normal">USDT</span>
            </p>
          </div>
          {totalPL !== 0 && (
            <div className={`flex items-center gap-1.5 text-sm font-medium ${totalPL >= 0 ? 'text-emerald-600' : 'text-red-600'}`}>
              {totalPL >= 0 ? <TrendingUp className="w-4 h-4" /> : <TrendingDown className="w-4 h-4" />}
              <span>{totalPL >= 0 ? '+' : ''}{formatCurrency(totalPL)} all time</span>
            </div>
          )}
        </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 animate-fade-in" style={{ animationDelay: '0.15s' }}>
          <div className="bg-white rounded-xl shadow-sm border border-sky-100 p-4">
            ...
            <p className="text-gray-400 text-xs font-medium uppercase tracking-wider">Account Balance</p>
            <p className="font-heading text-xl font-bold text-gray-800 mt-1">{formatCurrency(balance)}</p>
          </div>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-sky-100 p-4">
            ...
            <p className="text-gray-400 text-xs font-medium uppercase tracking-wider">Total Trades</p>
            <p className="font-heading text-xl font-bold text-gray-800 mt-1">{totalTrades}</p>
          </div>

          <div className="bg-white rounded-xl shadow-sm border border-sky-100 p-4">
            <div className={`w-10 h-10 rounded-full flex items-center justify-center mb-3 ${totalPL >= 0 ? 'bg-emerald-50' : 'bg-red-50'}`}>
              {totalPL >= 0 ? <TrendingUp className="w-5 h-5 text-emerald-500" /> : <TrendingDown className="w-5 h-5 text-red-500" />}
            </div>
            <p className="text-gray-400 text-xs font-medium uppercase tracking-wider">Total P/L</p>
            <p className={`font-heading text-xl font-bold mt-1 ${totalPL >= 0 ? 'text-emerald-600' : 'text-red-600'}`}>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-sky-100 p-4">
            ...
            <p className="text-gray-400 text-xs font-medium uppercase tracking-wider">Total Deposited</p>
            <p className="font-heading text-xl font-bold text-gray-800 mt-1">{formatCurrency(totalDeposited)}</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 animate-fade-in" style={{ animationDelay: '0.2s' }}>
          <div className="lg:col-span-2">
            <div className="flex items-center justify-between mb-4">
              <div>
                <h2 className="font-heading text-xl font-bold text-gray-800">Live Markets</h2>
              </div>
              <div className="flex items-center gap-2 px-3 py-1 rounded-full bg-emerald-50 border border-emerald-200">
                ...
                <span className="text-gray-400 text-xs">· Updates every 15s</span>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
              {Object.keys(CRYPTO_CONFIG).map((symbol) => {
                ...
                const change = coin.change_24h ?? coin.change ?? 0;
                const price = coin.price ?? coin.last ?? 0;
                const volume = coin.volume ?? coin.quote_volume ?? 0;
                const high24 = coin.high_24h ?? coin.high ?? 0;
                  ...
                  >
                    <div className="flex items-center gap-3 mb-3">
                      <div className={`w-9 h-9 rounded-xl ${cfg.bg} flex items-center justify-center flex-shrink-0`}>
                        <span className={`text-xs font-bold ${cfg.color}`}>{symbol.slice(0, 2)}</span>
                      </div>
                      <div className="min-w-0">
                        <p className="text-sm font-semibold text-gray-800 truncate">{symbol}/USDT</p>
                        <p className="text-xs text-gray-400 truncate">{cfg.label}</p>
                      </div>
                    </div>
                    <div className={`flex items-center gap-1 text-xs font-medium mb-2 ${isPositive ? 'text-emerald-600' : 'text-red-600'}`}>
                      {isPositive ? <TrendingUp className="w-3 h-3" /> : <TrendingDown className="w-3 h-3" />}
                      <span>{isPositive ? '+' : ''}{typeof change === 'number' ? change.toFixed(2) : '0.00'}%</span>
                    </div>
                    ...
                    <div className="space-y-1">
                      <div className="flex justify-between text-xs">
                        ...
                        <span className="text-gray-600">{formatNumber(volume)}</span>
                      </div>
                      <div className="flex justify-between text-xs">
                        <span className="text-gray-400">24h High</span>
                        <span className="text-gray-600">{high24 > 0 ? formatCurrency(high24) : '-'}</span>
                      </div>
                    </div>
                  </button>
                );
            ...
            <div className="bg-white rounded-xl shadow-sm border border-sky-100 p-4">
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-2">
                  ...
                  <div className="min-w-0">
                    <p className="text-gray-400 text-xs font-medium uppercase tracking-wider">Live Chart</p>
                    <p className="font-heading text-lg font-bold text-gray-800 truncate">{selectedCrypto}/USDT</p>
                  </div>
                </div>
              ...
              {chartLoading ? (
                ...
                <ResponsiveContainer width="100%" height={200}>
                  <AreaChart data={chartData} margin={{ top: 5, right: 5, left: 0, bottom: 0 }}>
                    <defs>
                      <linearGradient id="skyChartGradient" x1="0" y1="0" x2="0" y2="1">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 animate-fade-in" style={{ animationDelay: '0.25s' }}>
          <div className="bg-white rounded-xl shadow-sm border border-sky-100 p-6">
            <div className="flex items-center gap-2 mb-4">
              <Activity className="w-5 h-5 text-sky-500" />
              <h3 className="font-heading text-lg font-bold text-gray-800">Active Trades</h3>
            ...
            {activeTrades.length === 0 ? (
              <div className="text-center py-8">
                ...
                <p className="text-gray-300 text-xs mt-1">Start trading to see your positions here</p>
              </div>
            ) : (
              <div className="space-y-3">
                {activeTrades.slice(0, 6).map((trade) => {
                  const symbol = trade.crypto || trade.symbol || 'N/A';
                  const side = trade.direction || trade.side || 'open';
                  const isBuy = side === 'buy';
                  const remaining = countdowns[trade.id || trade._id] ?? 0;
                  const isUrgent = remaining < 10 && remaining > 0;

                  return (
                    <div key={trade.id || trade._id} className="flex items-center justify-between p-3 rounded-xl bg-sky-50/50 border border-sky-100">
                      <div className="flex items-center gap-3 min-w-0">
                        <div className={`w-9 h-9 rounded-lg flex items-center justify-center flex-shrink-0 ${isBuy ? 'bg-emerald-100' : 'bg-red-100'}`}>
                          {isBuy ? (
                        ...
                        <div className="min-w-0">
                          ...
                          <p className="text-xs text-gray-400">
                            <span className={isBuy ? 'text-emerald-600' : 'text-red-600'}>{side.toUpperCase()}</span>
                            {' · '}{formatCurrency(trade.amount || 0)}
                            {trade.profit_percent ? ` · +${trade.profit_percent}%` : ''}
                          </p>
                        </div>
                      </div>
                      <div className={`flex items-center gap-1.5 flex-shrink-0 ${isUrgent ? 'text-red-500' : 'text-gray-400'}`}>
                        <Clock className={`w-4 h-4 ${isUrgent ? 'animate-pulse' : ''}`} />
                        <span className="font-mono text-sm font-semibold">{remaining}s</span>
                      </div>
                    </div>
                  );
                })}
          ...
          <div className="bg-white rounded-xl shadow-sm border border-sky-100 p-6">
            <div className="flex items-center gap-2 mb-4">
              ...
              <h3 className="font-heading text-lg font-bold text-gray-800">Recent Trades</h3>
            </div>
            {tradeHistory.length === 0 ? (
              ...
              <div className="space-y-3">
                {tradeHistory.slice(0, 6).map((trade) => {
                  ...
                  const won = trade.status === 'won' || trade.status === 'profit';
                  const lost = trade.status === 'lost' || trade.status === 'loss';
                  const date = trade.created_at
                    ? new Date(trade.created_at).toLocaleDateString('en-US', { month: 'short', day: 'numeric' })
                    : '';

                  return (
                    <div key={trade.id || trade._id} className="flex items-center justify-between p-3 rounded-xl bg-gray-50 border border-gray-100">
                      <div className="flex items-center gap-3 min-w-0">
                        <div className={`w-9 h-9 rounded-lg flex items-center justify-center flex-shrink-0 ${won ? 'bg-emerald-100' : lost ? 'bg-red-100' : 'bg-sky-100'}`}>
                          {won ? (
                        ...
                        <div className="min-w-0">
                          ...
                          <p className="text-xs text-gray-400 capitalize">
                            {trade.direction || trade.side} · {date || 'Just now'}
                          </p>
                        </div>
                      </div>
                      <div className="text-right flex-shrink-0">
                        <p className={`text-sm font-semibold ${pnl >= 0 ? 'text-emerald-600' : 'text-red-600'}`}>
                          {pnl >= 0 ? '+' : ''}{formatCurrency(pnl)}
                        </p>
                        <span className={`inline-flex px-2 py-0.5 rounded-full text-[10px] font-semibold ${won ? 'bg-emerald-50 text-emerald-600' : lost ? 'bg-red-50 text-red-600' : 'bg-sky-50 text-sky-600'}`}>
                          {trade.status || 'active'}
                        </span>
                      </div>
                    </div>
        ...
        <div className="animate-fade-in" style={{ animationDelay: '0.3s' }}>
          <div className="flex items-center gap-2 mb-4">
            <div className="w-1.5 h-5 rounded-full bg-sky-500" />
            <h3 className="font-heading text-lg font-bold text-gray-800">Market News</h3>
          </div>
